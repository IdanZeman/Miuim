...
CREATE OR REPLACE FUNCTION public.restore_snapshot(
  ...
  p_operation text DEFAULT 'both' -- 'both', 'delete_only', 'insert_only'
)
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  -- 3. Cleanup Phase (Delete existing data)
  IF p_operation = 'both' OR p_operation = 'delete_only' THEN
    -- Target ID Collection
    CREATE TEMP TABLE IF NOT EXISTS target_teams AS SELECT id::text FROM public.teams WHERE organization_id::text = p_organization_id::text;
    CREATE TEMP TABLE IF NOT EXISTS target_roles AS SELECT id::text FROM public.roles WHERE organization_id::text = p_organization_id::text;
    CREATE TEMP TABLE IF NOT EXISTS target_tasks AS SELECT id::text FROM public.task_templates WHERE organization_id::text = p_organization_id::text;
    CREATE TEMP TABLE IF NOT EXISTS target_people AS SELECT id::text FROM public.people WHERE organization_id::text = p_organization_id::text OR team_id::text IN (SELECT id FROM target_teams);
    CREATE TEMP TABLE IF NOT EXISTS target_shifts AS SELECT id::text FROM public.shifts WHERE organization_id::text = p_organization_id::text OR task_id::text IN (SELECT id FROM target_tasks);
    CREATE TEMP TABLE IF NOT EXISTS target_equipment AS SELECT id::text FROM public.equipment WHERE organization_id::text = p_organization_id::text;

    -- Selective Deletion (respecting FK order)
    ...
    DROP TABLE IF EXISTS target_teams;
    DROP TABLE IF EXISTS target_roles;
    DROP TABLE IF EXISTS target_tasks;
    DROP TABLE IF EXISTS target_people;
    DROP TABLE IF EXISTS target_shifts;
    DROP TABLE IF EXISTS target_equipment;
  END IF;

  -- 4. Restore Phase (Insert data)
  IF p_operation = 'both' OR p_operation = 'insert_only' THEN
    FOR v_table_record IN (
      ...
      ELSIF v_table_name = 'unified_presence' THEN
        ...
        SELECT (x->>'id')::uuid, p_organization_id, (x->>'person_id')::text, (x->>'date')::date, (x->>'status')::text, (x->>'start_time')::text, (x->>'end_time')::text, (x->>'source')::text, (x->>'source_id')::text, (x->>'created_at')::timestamp with time zone, (x->>'updated_at')::timestamp with time zone, (x->>'reason')::text, (x->>'last_editor_id')::uuid, (x->>'arrival_date')::timestamp with time zone, (x->>'departure_date')::timestamp with time zone, (x->>'home_status_type')::text
        FROM jsonb_array_elements(v_data) AS x;
